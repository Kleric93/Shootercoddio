	...
	DeathTime(10.f)
{
	...
	//HitNumbers = CreateDefaultSubobject<UWidgetComponent>(TEXT("NumbersWidget"));
	//HitNumbers->SetupAttachment(RootComponent);
}
...
void AEnemy::BeginPlay()
{
	...
	// ignore the camera for mesh and capsule to avoid glitching when attacking
...
void AEnemy::AgroSphereOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (Character)
	{
		if (EnemyController)
		{
			...
			{
				// set the values of the Target Blackboard Key
...
float AEnemy::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (Stunned <= StunChance)
	{
		...
		//bStunned = true; WAS SWAPPED WITH BELOW
		SetStunned(true);
	}
	return DamageAmount;
}
